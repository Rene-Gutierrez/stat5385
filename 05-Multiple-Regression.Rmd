...
Multiple linear regression is a fundamental statistical technique used to model the relationship between a single dependent variable and two or more independent variables. It generalizes simple linear regression by allowing several predictors to jointly explain variation in the outcome.  
...
- $\beta_1, \beta_2, \ldots, \beta_p$ are the coefficients of the predictors $x_{1,i}, x_{2,i}, \ldots, x_{p,i}$, and  
- $e_i$ is the random error term.  

This framework is widely applied in economics, the social sciences, and many other fields to control for multiple factors simultaneously and to improve predictive accuracy. However, it also brings additional challenges beyond those in simple regression, such as multicollinearity, the choice of relevant predictors, and the validity of standard assumptions.

We have already seen a form of multiple regression in the context of polynomial regression. Here, however, we consider the general case with different explanatory variables.  
